...
void MtcaProgrammerBase::initRegisterAccessors(const std::string& registerPathName)
{
    ...
    reg_bytes_to_write.replace(mDevice.getScalarRegisterAccessor<int32_t>(registerPathName, REG_BYTES_TO_WRITE, {ChimeraTK::AccessMode::raw}));
    reg_bytes_to_read.replace(mDevice.getScalarRegisterAccessor<int32_t>(registerPathName, REG_BYTES_TO_READ, {ChimeraTK::AccessMode::raw}));
    reg_control.replace(mDevice.getScalarRegisterAccessor<int32_t>(registerPathName, REG_CONTROL, {ChimeraTK::AccessMode::raw}));
    ...
    reg_rev_switch.replace(mDevice.getScalarRegisterAccessor<int32_t>(registerPathName, REG_REV_SWITCH, {ChimeraTK::AccessMode::raw}));
}
