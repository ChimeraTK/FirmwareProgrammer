...
boost::shared_ptr<mtca4u::devBase> mtca_dev_for_ports;
...
{    
	if(!dummy_xsvf_player)
	{
		...
		{
			if (val == 1)
                                mtca_dev_for_ports->writeReg(regAddress(tms) + OFFSET, 0x1, BAR);
				//dev.writeReg(regAddress(tms)+OFFSET, 0x1, BAR_NR);
				//writeRegister (sxvfInfo.devDescriptor, tms, 0x1);
			else
                                mtca_dev_for_ports->writeReg(regAddress(tms) + OFFSET, 0x0, BAR);
		...
		{
			if (val == 1)
                                mtca_dev_for_ports->writeReg(regAddress(tdi)+OFFSET, 0x1, BAR);
				//dev.writeReg(regAddress(tdi)+OFFSET, 0x1, BAR_NR);   
				//writeRegister (sxvfInfo.devDescriptor, tdi, 0x1);
			else
                                mtca_dev_for_ports->writeReg(regAddress(tdi)+OFFSET, 0x0, BAR);
		...
		{
			if (val == 1)
                                mtca_dev_for_ports->writeReg(regAddress(tck)+OFFSET, 0x1, BAR);
				//dev.writeReg(regAddress(tck)+OFFSET, 0x1, BAR_NR); 
				//writeRegister (sxvfInfo.devDescriptor, tck, 0x1);
			else
                                mtca_dev_for_ports->writeReg(regAddress(tck)+OFFSET, 0x0, BAR);
...
//   usleep(1);
	//readRegister (sxvfInfo.devDescriptor, tdo, &data);
        mtca_dev_for_ports->readReg(regAddress(tdo)+OFFSET, &data, BAR);
