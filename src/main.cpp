...
struct arguments_t {
  ...
  arguments_t()
      : interface(ProgrammingInterface(ProgrammingInterface::INTERFACE_NONE)),
        firmware_file_path(), device_name(), device_name_raw(false),
        address(PROG_DEFAULT_ADDRESS), bar(PROG_DEFAULT_BAR), dmap_file_path(),
        map_file_path(), map_area_name("**DEFAULT**"),
        action_programming(false), action_verification(false),
        action_reload(false) {}
...
arguments_t parse_arguments(int argc, char *argv[]) {
  ...
  config.add_options()("program,p", po::bool_switch()->default_value(false),
                       "erase and program memory")(
      ...
      "firmware_file,f", po::value<string>(),
      "FPGA firmware file")("device,d", po::value<string>(), "device name")(
      "address,a", po::value<string>(),
      "address and bar of boot area in FGPA - example: -a [address]b[bar]")(
      "dmap,D", po::value<string>(),
      "DMAP file path")("map,M", po::value<string>(), "MAP file path")(
  ...
  if (vm.count("config")) {
    ...
    po::store(po::parse_config_file<char>(config_file_path.c_str(),
                                          config_file_options),
              vm);
    po::notify(vm);
  }
  ...
  if (vm.count("interface")) {
    ...
    if (interface.empty())
      args.interface =
          ProgrammingInterface(ProgrammingInterface::INTERFACE_NONE);
    else if (interface.compare("spi") == 0)
      args.interface =
          ProgrammingInterface(ProgrammingInterface::INTERFACE_SPI);
    else if (interface.compare("jtag") == 0)
      args.interface =
          ProgrammingInterface(ProgrammingInterface::INTERFACE_JTAG);
    else
  ...
  if (vm.count("address")) {
    ...
    try {
      ...
      std::string bar_ch = address_input.substr(
          b_position + 1, address_input.length() - b_position);
...
void verify_arguments(arguments_t arguments) {
  if (!arguments.action_programming && !arguments.action_reload &&
  ...
  if (arguments.interface.getType() == ProgrammingInterface::INTERFACE_NONE) {
    throw std::logic_error(
        "Programming interface (SPI/JTAG) is missing.\n"
  ...
  if (arguments.firmware_file_path.empty()) {
    ...
    if (tmp_file == NULL)
      throw std::invalid_argument(
          "Cannot open firmware file. Please check if file exists and you have "
          "rights to access it.\n");
    else
      fclose(tmp_file);
  }
  ...
  try {
    ...
    if (!arguments.dmap_file_path.empty()) // DMAP mode
    {
      ...
      case ProgrammingInterface::INTERFACE_SPI:
        programmer =
            boost::shared_ptr<MtcaProgrammerBase>(new MtcaProgrammerSPI(
                ProgAccessDmap(arguments.device_name, arguments.dmap_file_path,
                               arguments.map_area_name)));
        break;
      case ProgrammingInterface::INTERFACE_JTAG:
        programmer =
            boost::shared_ptr<MtcaProgrammerBase>(new MtcaProgrammerJTAG(
                ProgAccessDmap(arguments.device_name, arguments.dmap_file_path,
                               arguments.map_area_name)));
        break;
      default:
    ...
    } else if (!arguments.map_file_path.empty()) // MAP mode
    {
      ...
      case ProgrammingInterface::INTERFACE_SPI:
        programmer =
            boost::shared_ptr<MtcaProgrammerBase>(new MtcaProgrammerSPI(
                ProgAccessMap(arguments.device_name, arguments.map_file_path,
                              arguments.map_area_name)));
        break;
      case ProgrammingInterface::INTERFACE_JTAG:
        programmer =
    ...
    } else // RAW mode
    {
      ...
      case ProgrammingInterface::INTERFACE_SPI:
        programmer = boost::shared_ptr<MtcaProgrammerBase>(
            new MtcaProgrammerSPI(ProgAccessRaw(
                arguments.device_name, arguments.bar, arguments.address)));
        break;
      case ProgrammingInterface::INTERFACE_JTAG:
        programmer = boost::shared_ptr<MtcaProgrammerBase>(
            new MtcaProgrammerJTAG(ProgAccessRaw(
                arguments.device_name, arguments.bar, arguments.address)));
        break;
      default:
        throw std::invalid_argument("Unknown interface\n\n");
